...
import { AsyncUserUnsubscribesFromCardUseCase } from '../../src/use-cases/AsyncUserUnsubscribesFromCardUseCase.js'
...
userUnsubscribesFromCard('given an existing user id subscribed to an existing cardid, then unsubscribe', async () => {
    ...
    const datastore = ImplementationsContainer.get(Dependency.ASYNC_DATASTORE) as AsyncDatastore
    ...
    await new AsyncUserSubscribesToCardUseCase().execute(request)
    await new AsyncUserUnsubscribesFromCardUseCase().execute(request)
...
userUnsubscribesFromCard('given an existing user id subscribed to an existing cardid, then return a ok withouth data response', async () => {
    ...
    const result = await new AsyncUserUnsubscribesFromCardUseCase().execute(request)
